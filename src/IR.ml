...
type value =
  ...
  | Proj of value * field (* Immutable field access in value language *)
...
let pp origc =
  ...
  let rec value env () = function
    ...
    | Var v ->
       pp "%s" (deref vname v)
    | Proj (v,f) ->
       pp "%a.%s" (value env) v (field_name f)
    | Tuple (tag, fields) ->
  ...
  and comp env () = function
    ...
    | Match (v, cases, def) ->
       let cases =
         List.map (fun (t, c) -> [t], c) cases @ Option.to_list def
       in
       let ptags = function
